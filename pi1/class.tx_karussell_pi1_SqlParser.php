 ...
 * @version 0.0.4
 */
class tx_karussell_pi1_SqlParser {

    var $this = null;
    
    // statements 
    public $querysections = array('alter','create','drop', 
    ...
    // types
    public $types = array('character', 'char', 'varchar', 'nchar', 
    ...
    // conjuctions
    public $conjuctions = array('by', 'as', 'on', 'into', 
                                       'from', 'where', 'with');
    
    // basic functions
    public $funcitons = array('avg', 'count', 'max', 'min', 
                                     'sum', 'nextval', 'currval', 'concat',
                                     );
    
    // reserved keywords
    public $reserved = array('absolute', 'action', 'add', 'all', 
     ...
     * @return token array
     */
    public function Tokenize($sqlQuery, $cleanWhitespace = true) {
     ...
     */
    public function ParseString($sqlQuery, $cleanWhitespace = true) {

        // instantiate if called statically
    /*    if (!isset($this)) {
            $this = ''; // new tx_karussell_pi1_SqlParser();
        } else {
            $this = $this;
        }
*/
        // copy and tokenize the query
        $tokens = $this->Tokenize($sqlQuery, $cleanWhitespace); // $this->Tokenize($sqlQuery, $cleanWhitespace);
        ...
        for ($t = 0; $t < $tokenCount; $t++) {
            // if is paren
            if (in_array($tokens[$t], $this->startparens)) {
                ...
                if (in_array(strtolower($tokens[$t]), $this->querysections) && !isset($this->query[$tokens[$t]])) {
                    $section = strtolower($tokens[$t]);
                }

                // rebuild the query in sections
                if (!isset($this->query[$section]))
                    $this->query[$section] = '';
    ...
    private function readsub($tokens, &$position) {
        ...
        $tokenCount = count($tokens);
        $position++;
        while (!in_array($tokens[$position], $this->endparens) && $position < $tokenCount) {

            if (in_array($tokens[$position], $this->startparens)) {
