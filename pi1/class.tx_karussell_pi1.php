...
*  Copyright notice
*
*  (c) 2011 Kurt Gusbeth <k.gusbeth@fixpunkt.com>
*  All rights reserved
*
...
class tx_karussell_pi1 extends tslib_pibase {
	...
	function main($content, $conf)	{
		...
		if (is_array(t3lib_div::_GP($this->prefixId))) {
		  ...
		  $this->showUid = addslashes($params['showUid']);
		  if (!$this->showUid) $this->showUid=0;
		}
		...
		if ($this->conf['table'] && $this->conf['searchFieldList']) {
			...
			$this->searchFieldList = $this->table.'.uid,'.$this->conf['searchFieldList'];
			$this->dirName = $this->conf['images.']['dirName'];
		} else {
	...
	function listView() {	//$content, $conf) {
		...
		if ($this->conf['sort']!='rand()') {
			list($this->internal['orderBy'],$this->internal['descFlag']) = explode(':',$this->conf['sort']);	// statt $this->piVars['sort']
		...
		$andWhere = preg_replace("/###LANG_UID###/s", $this->lang, $this->conf['andWhere']);
		if ($andWhere) {
...
//        $tRows[] = $this->pi_list_header();
	...
	function pi_list_row($c,$isJS,$isControl,$inr='') {
		...
		if ($this->table<>'tx_karussell_inhalt' && $this->conf['parameterUID'] && $this->conf['destinationPID']) {
			$typolink_conf = array(
				'parameter' => $this->conf['destinationPID'],
				'additionalParams' => '&'.$this->conf['parameterUID'].'='.$uid,
				'useCacheHash' => 1);
			$linkTag = $this->cObj->typolink($this->conf['linkText'], $typolink_conf);
			$link = $this->pi_getPageLink($this->conf['destinationPID'], '', array($this->conf['parameterUID'] => $uid));
		} else if ($link) {
		...
		if ($isJS) {
			...
			if ($this->conf['foreignSelect']) {
				$query = $this->cObj->substituteMarkerArray($this->conf['foreignSelect'], $markerArray);
